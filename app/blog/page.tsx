import { databaseUrlStatus, prisma } from "@/lib/db";
...
export default async function BlogIndexPage() {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      console.warn(`Database not available during build: ${message}`);
    }
    posts = [];
  }
      ...
      <ul className="space-y-8">
        {posts.map((p: Post) => (
          ...
          >
            ...
            {p.excerpt && (
              <p className="text-neutral-400 mb-2 line-clamp-3">{p.excerpt}</p>
            )}
        ...
        {posts.length === 0 && (
          <li className="text-neutral-500">No articles yet.</li>
        )}
      </ul>
    </main>
  );
