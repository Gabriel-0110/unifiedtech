...
export const withErrorBoundary =
  (
    handler: (req: AugmentedRequest) => Promise<NextResponse> | NextResponse
  ): Middleware =>
  async (req) => {
    try {
...
export const sanitizeInput = {
  email: (email: unknown) =>
    typeof email === "string"
      ...
      : "",
  string: (str: unknown, max = 1000) =>
    typeof str === "string"
      ? validator.escape(str.trim().substring(0, max))
      : "",
};
